    ...
    if ('observed_signature' in node) {
      ...
      if (reads !== undefined) {
        ...
        for (const one of many) {
          ...
          // Четвёртая форма - ?имя: значение лежит параметром строки запроса
          // внутри того адреса, который объявлен узлом выше. Заведена
          // 31.08.2026, когда формат скелета v9 перестал маскировать имена
          // параметров.
          //
          // До него объявить такое правило было НЕЧЕМ: строка запроса уходила в
          // снимок одной подписью, и проверить объявление по снимку значило бы
          // гадать. Идентификатор чужого предложения лежит ровно там и больше
          // нигде.
          if (
            one !== 'text' &&
            !/^@[\w-]+$/.test(String(one)) &&
            !/^\?[A-Za-z_][A-Za-z0-9_-]*$/.test(String(one)) &&
            !/^\$[A-Za-z_][A-Za-z0-9_-]*(\.[A-Za-z_][A-Za-z0-9_-]*)*$/.test(String(one))
          ) {
            fail(rel, `${at}: reads «${one}» не разбирается. Ожидается text, ` +
              '@имя-атрибута, ?имя-параметра запроса либо $путь.до.ключа внутри JSON атрибута')
          }
        }
      }
